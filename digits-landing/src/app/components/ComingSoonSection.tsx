            ...
            <div key={index} className="relative">
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-8 border border-gray-200/50 hover:bg-white hover:shadow-xl transition-all duration-300 relative z-10 group">
                <div className="text-center mb-6">
                  <div className="w-20 h-20 bg-gradient-to-br from-orange/10 to-blue/10 rounded-2xl flex items-center justify-center mx-auto mb-4 text-4xl transform group-hover:scale-110 transition-transform duration-300 border border-gray-200/50">
                    {feature.icon}
                  </div>
                  <div className={`inline-block px-4 py-2 rounded-full text-sm font-semibold mb-2 border ${getStatusColor(feature.status)}`}>
                    {feature.status}
                  </div>
                ...
                <ul className="space-y-3">
                  {feature.features.map((item, itemIndex) => (
                    <li key={itemIndex} className="flex items-center">
                      <div className="w-5 h-5 bg-gradient-to-r from-orange to-orange-500 rounded-full flex items-center justify-center mr-3 flex-shrink-0">
